...
export class Result implements OnInit, OnDestroy {
  ...
  calculateBoxScoreStats(data: MatchData) {
    ...
    const homeStats = { ...statsTemplate };
    const awayStats = { ...statsTemplate };

    // Go through every event and update stats
    for (const event of data.events) {
      ...
      const targetStats =
        team === data.teams.home.name ? homeStats : awayStats;

      if (type === 'goal') {
        targetStats.GoalsScored++;
        targetStats.FieldGoalsScored++; // assuming every "Goal" is a field goal unless explicitly marked otherwise
      } else if (type === 'penalty corner scored') {
        targetStats.PenaltyCornersScored++;
        targetStats.GoalsScored++;
      } else if (type === 'penalty stroke scored') {
        targetStats.PenaltyStrokesScored++;
        targetStats.GoalsScored++;
      } else if (type === 'penalty stroke missed') {
        targetStats.PenaltyStrokesMissed++;
      } else if (type === 'green card') {
        targetStats.GreenCards++;
      } else if (type === 'yellow card') {
        targetStats.YellowCards++;
      } else if (type === 'red card') {
        targetStats.RedCards++;
      }
    }

    // Assign computed stats back to data
  ...
  private fetchMatchData(matchId: string): void {
    ...
    this.http.get<any>(url).subscribe({
      next: (data) => {
        ...
        this.matchData = this.calculateBoxScoreStats(this.matchData);
